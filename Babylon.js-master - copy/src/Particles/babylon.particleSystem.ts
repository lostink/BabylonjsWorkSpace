module BABYLON {
    ...
    export class ParticleSystem implements IDisposable, IAnimatable, IParticleSystem {
        ...
        //My own attributes
        public sphereEmitRadius: number = 1.0;
        public coneEmitRadius: number = 5.0;
        public coneEmitAngle : number = Math.PI / 10.0;
        private _emissionShapeType:number = 1;
        public get emissionShapeType() : number{
            return this._emissionShapeType;
        }
        public set emissionShapeType(value:number){
            if (this._emissionShapeType == value){
                return;
            }
            if ((value != 1) && (value != 2) && (value != 3)){
                return;
            }
            this._emissionShapeType = value;
            if (this.emissionShapeType == 1){
            ...
            if (this.emissionShapeType == 2){ // Sphere
                this.startDirectionFunction = (emitPower: number, worldMatrix: Matrix, directionToUpdate: Vector3, particle: Particle): void => {
                    var theta = randomNumber(0.0, Math.PI);
                    var fai   = randomNumber(0.0, Math.PI * 2.0);
                    var randX = Math.sin(theta) * Math.cos(fai);
                    var randY = Math.sin(theta) * Math.sin(fai);
                    var randZ = Math.cos(theta);
                    Vector3.TransformNormalFromFloatsToRef(randX * emitPower, randY * emitPower, randZ * emitPower, worldMatrix, directionToUpdate);
                }
                this.startPositionFunction = (worldMatrix: Matrix, positionToUpdate: Vector3, particle: Particle): void => {
                    var randX = 0.0;
            ...
            else if (this.emissionShapeType == 3){ //Cone
                this.startDirectionFunction = (emitPower: number, worldMatrix: Matrix, directionToUpdate: Vector3, particle: Particle): void => {
                    //coneEmitRadius
                    var theta = this.coneEmitAngle;
                    var fai   = randomNumber(0.0, Math.PI * 2.0);
                    var disk  = randomNumber(0.0,1.0);
                    var randX = Math.sin(theta) * Math.cos(fai) * disk;
                    var randY = Math.sin(theta) * Math.sin(fai) * disk;
                    var randZ = Math.cos(theta);
        ...
        */
        ...
        constructor(public name: string, capacity: number, scene: Scene, customEffect: Nullable<Effect> = null, private _isAnimationSheetEnabled: boolean = false, epsilon: number = 0.01, emissionType: number = 1) {
            this.id = name;
